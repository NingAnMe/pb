...
class CLASS_VIIRS_L1():
    ...
    def __init__(self):
        ...
        self.LandCover = None
        self.WN = {'CH_08':8064.516, 'CH_09':7256.894, 'CH_10':6211.180, 'CH_11':4444.444, 'CH_12':2702.702, 'CH_13':2469.135, 'CH_14':1169.590, 'CH_15':929.108, 'CH_16': 832.431}
        self.TeA = {'CH_15':1.0}
        self.TeB = {'CH_15':0}
    ...
    def Load(self, L1File):
        ...
        try:
            ...
            ary_satz = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SatelliteZenithAngle')[:]
            ary_sata = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SatelliteAzimuthAngle')[:]
            ary_sunz = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SolarZenithAngle')[:]
            ary_suna = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SolarAzimuthAngle')[:]
            ...
            for i in xrange(1, 17, 1):
                if i < 12:
                    Ref[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Reflectance' % i)[:]

                elif i == 13:
                    Rad[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Radiance' % i)[:]
                    Tbb[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperature' % i)[:]
                    Rad_a[i] = 1.
                    Rad_b[i] = 0.
                    Tbb_a[i] = 1.
                    Tbb_b[i] = 0.
                    ...
                    Rad[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Radiance' % i)[:]
                    Rad_a[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/RadianceFactors' % i)[:][0]
                    Rad_b[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/RadianceFactors' % i)[:][1]
                    Tbb[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperature' % i)[:]
                    Tbb_a[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperatureFactors' % i)[:][0]
                    Tbb_b[i] = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperatureFactors' % i)[:][1]
...
#         for i in xrange(self.Band):
        ...
        for i in range(1, 12, 1):
            ...
            if BandName not in self.Ref.keys():
                ...
                self.Ref[BandName] = np.concatenate((self.Ref[BandName], Ref_data))

            self.SV[BandName] = None
            self.BB[BandName] = None
        # 红外
        for i in xrange(12, 17, 1):
            ...
            Rad_data[idx] = Rad[i][idx] * ((10000 / self.WN[BandName]) ** 2) / 10.
            ...
            if BandName not in self.Rad.keys():
                ...
                self.Rad[BandName] = np.concatenate((self.Rad[BandName], Rad_data))

            if BandName not in self.Tbb.keys():
                self.Tbb[BandName] = Tbb_data
            else:
                self.Tbb[BandName] = np.concatenate((self.Tbb[BandName], Tbb_data))
