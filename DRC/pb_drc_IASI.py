...
class ReadIasiL1(ReadL1):
    ...
    def set_file_attr(self):
        ...
        if self.resolution == 24000:
            satellite_type1 = ['METOP-A', 'METOP-A']
        ...
        if self.resolution == 24000:
            ...
            if self.satellite in satellite_type1:
                try:
                    ...
                    self.data_shape = (record.time.shape[0], 1)
                except Exception as e:
        ...
        if self.resolution == 24000:
            ...
            if self.satellite in satellite_type1:
                try:
                    record = beatl2.ingest(self.in_file)
                    # 单位转换
                    response = record.spectral_radiance * 10 ** 7
                    # 暂时取一个观测的光谱波数
                    wave_number = record.wavenumber[0, :]
