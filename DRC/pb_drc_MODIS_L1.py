...
#             in_data_LandSeaMask = h4File.select('Land/SeaMask').get()  # 多个/不知道是bug还是就这样
        ...
        for i in  range(0, 2, 1):
            # 过滤 无效值
            condition = np.logical_and(in_data_r250[i] < 32767, in_data_r250[i] > 0)
            idx = np.where(condition)
            data_ch38[i][idx] = (in_data_r250[i][idx] - in_data_r250_o[i]) * in_data_r250_s[i]
        # 3-7通道
        for i in range(2, 7, 1):
            # 过滤 无效值
            condition = np.logical_and(in_data_r500[i - 2] < 32767, in_data_r500[i - 2 ] > 0)
            idx = np.where(condition)
            data_ch38[i][idx] = (in_data_r500[i - 2][idx] - in_data_r500_o[i - 2]) * in_data_r500_s[i - 2]
        # 8-19 外加一个 26通道
        for i in range(7, 22, 1):
            # 过滤 无效值
            condition = np.logical_and(in_data_r1km[i - 7] < 32767, in_data_r1km[i - 7] > 0)
            idx = np.where(condition)
            data_ch38[i][idx] = (in_data_r1km[i - 7][idx] - in_data_r1km_o[i - 7]) * in_data_r1km_s[i - 7]
        # 20-36通道  不包括26通道
        for i in range(22, 38, 1):
            # 过滤 无效值
            condition = np.logical_and(in_data_t1km[i - 22] < 32767, in_data_t1km[i - 22] > 0)
            idx = np.where(condition)
            data_ch38[i][idx] = (in_data_t1km[i - 22][idx] - in_data_t1km_o[i - 22]) * in_data_t1km_s[i - 22]
...
#         print info.dt_s
        ...
        for i in xrange(16):
            newRad[i] = data_ch36[i + 20] * ((10000 / cwn[i]) ** 2) / 10. * dsol
            if i <= 5:
...
#         order_data_ch36 = np.insert(del26_data_ch35, 25, data_ch36[19], 0)
        ...
        for i in range(0, 36, 1):
            ...
            self.SV['CH_%02d' % (i + 1) ] = None
            self.BB['CH_%02d' % (i + 1) ] = None
...
if __name__ == '__main__':
    L1File = 'D:/nsmc/data/FY3_MODIS/MYD021KM.A2017003.0750.006.2017004153232.hdf'
    modis = CLASS_MODIS_L1()
    modis.Load(L1File)
#     print sorted(modis.Ref.keys())
    print sorted(modis.Rad.keys())
