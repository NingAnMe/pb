...
class CLASS_VIIRS_L1():
    ...
    def Load(self, L1File):
        ...
        try:
            ...
            self.satAzimuth = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SatelliteAzimuthAngle')[:]
            self.satZenith = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SatelliteZenithAngle')[:]
            self.sunAzimuth = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SolarAzimuthAngle')[:]
            self.sunZenith = h5File_R.get('/All_Data/VIIRS-MOD-GEO_All/SolarZenithAngle')[:]
...
#             print ymd
            ...
            for i in xrange(1, 17, 1):
                ...
                if i < 12:
                    ref = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Reflectance' % i)[:]
    #                 ref_fac = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/ReflectanceFactors' % i)[:][0]
                    ref_fac = 2.4417415E-5  # 由于不确定性 比较了14和15年的数据值一致，定值。偶尔出现-999.33导致计算错
                    idx = np.where(ref < 65500)
                    newRef = np.full_like(ref, np.nan, 'f4')
                    newRef[idx] = ref[idx]
                    self.Ref[bandName] = newRef * ref_fac * np.cos(np.deg2rad(self.sunZenith)) * dsol
                elif i == 13:
                    rad = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Radiance' % i)[:]
                    tbb = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperature' % i)[:]
                    idx = np.where(rad > 0)
                    newRad = np.full_like(rad, np.nan, 'f4')
                    newRad[idx] = rad[idx] * ((10000 / self.WN['CH_%02d' % i ]) ** 2) / 10.
                    ...
                    rad = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/Radiance' % i)[:]
                    rad_fac = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/RadianceFactors' % i)[:][0]
                    rad_int = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/RadianceFactors' % i)[:][1]
                    tbb = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperature' % i)[:]
                    tbb_fac = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperatureFactors' % i)[:][0]
                    tbb_int = h5File_R.get('/All_Data/VIIRS-M%d-SDR_All/BrightnessTemperatureFactors' % i)[:][1]
